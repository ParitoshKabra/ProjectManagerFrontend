...
function MakeAdmin(props) {
    ...
    const getMembers = async () => {
        const res = await axios
            .get('http://127.0.0.1:8000/trelloAPIs/project_members/' + props.project.id, {
        ...
        mems = members.map(item => {
            ...
            return (<ListItemButton role={undefined} onClick={handleToggle(item.id)} key={item.id} dense>
                ...
                <ListItemText
                    ...
                    secondary={props.project.admins.indexOf(item.id) > -1 ? (item.id === props.project.created_by ? 'creator' : 'admin') : 'member'}
                />
                <ListItemIcon>
                    <Checkbox
